...
import org.apache.storm.s3.output.SimpleS3TransactionalOutput;
...
public class S3StateFactory implements StateFactory {
    ...
    public State makeState(Map conf, IMetricsContext metrics, int partitionIndex, int numPartitions) {
        ...
        SimpleS3TransactionalOutput s3 = new SimpleS3TransactionalOutput(conf);
        try {
            String identifier = String.valueOf(partitionIndex);
            s3.withBucketName("global-state").withIdentifier(identifier);
            s3.prepare(conf);
        } catch (IOException e) {
